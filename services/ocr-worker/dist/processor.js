...
import { getDocument, replaceDocumentChunks, saveOcrTextAndDiagnostics, setDocumentOcrFailed, setDocumentOcrProcessing, updateDocumentOcrProgress, } from "./db.js";
import { embedDocumentChunks } from "./embeddings.js";
import { ocrTranscribePdf } from "./pdf-ocr.js";
function mergeSupplementText(existing, ocrText) {
...
export async function processOcrJob(payload, hooks) {
    ...
    try {
        ...
        console.log("[ocr-worker] processor: OCR text stats", {
            ...
            charsPerPage: ocr.pageCount && ocr.pageCount > 0 ? Math.round(text.length / ocr.pageCount) : null,
        });
