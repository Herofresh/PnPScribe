...
function chooseChunkEnd(
  ...
  for (const offset of preferredOffsets) {
    if (offset >= minBreakOffset) {
      ...
      if (end - start >= Math.min(minChunkSize, window.length)) return end;
    }
  }
...
function findChapterHint(content: string) {
  ...
  for (const line of lines) {
    if (/^(chapter|section|part)\b/i.test(line)) return line.slice(0, 140);
    if (/^[A-Z][A-Z0-9\s:,-]{4,80}$/.test(line)) return line.slice(0, 140);
  }
